...
<tr>
    ...
    <td class="text-center">
        ...
        @endif
        <br>
        <button class="btn btn-icon btn-{{ $warna }}" data-toggle="modal"
            data-target="#pilihguru{{ $dp->id }}_{{ $dk->id }}"
            id="btnpilihguru{{ $dp->id }}_{{ $dk->id }}">{{ substr($guru, 0, 7) }}</button>

        <br>
        @if($cekdatagurupengampu>0)
        <a href="{{ url('/admin/inputnilai/mapel') }}/{{ $dataajar->id }}" type="button" class="btn btn-outline-primary"
            data-toggle="tooltip" data-placement="top" title="Input nilai Mapel"><i
                class="fas fa-user-graduate"></i></a>
        {{-- <a href="{{ url('/admin/inputnilai/kepribadian') }}/{{ $dp->id }}/{{ $dk->id }}" type="button" class="btn
        btn-outline-primary" data-toggle="tooltip" data-placement="top" title="Input nilai Kepribadian"><i
            class="fas fa-ribbon"></i></a>
        <a href="{{ url('/admin/inputnilai/ekstrakulikuler') }}/{{ $dp->id }}/{{ $dk->id }}" type="button"
            class="btn btn-outline-primary" data-toggle="tooltip" data-placement="top"
            title="Input nilai Ekstrakulikuler"><i class="fas fa-dungeon"></i></a> --}}
        @endif
        @endif
    </td>
    @endforeach
...
@endif
@if($tombol!=='')
<div class="modal fade" id="pilihguru{{ $dp->id }}_{{ $dk->id }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control form-control-lg" id="tags{{ $dp->id }}_{{ $dk->id }}" select2
                            select2-hidden-accessible multiple="multiple" name="guru_nomerinduk" required>
                            @php
                            $cekdatagurupengampu = DB::table('dataajar')
                            ->where('kelas_nama', '=', $dk->nama)
                            ->where('pelajaran_nama', '=', $dp->nama)
                            ...
                            @endphp
                            @if($cekdatagurupengampu>0)
                            <option value="{{ $dataajar->guru_nomerinduk }}" selected>{{ $dataajar->guru_nama }}
                            </option>
                    ...
                    {{-- <select class="form-control form-control-lg" required name="guru_nomerinduk">
                                ...
                                $dataajar=DB::table('dataajar')
                                  ...
                                  @endphp
                                  @if($cekdatagurupengampu>0)
                                  <option value="{{ $dataajar->guru_nomerinduk }}">{{ $dataajar->guru_nomerinduk }} -
                    {{ $dataajar->guru_nama }}</option>
