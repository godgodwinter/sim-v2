      ...
      created: function() {
        ...
        @foreach ($datasiswa as $ds)
          @foreach ($datajenisnilai as $dj)
             this.getPeriksamapel{{ $ds->id }}_{{ $dj->id }}();
          @endforeach
          @endforeach
      },
      methods: {
          ...
          getProducts: function () {
              axios.get('http://127.0.0.1:8000/api/admin/inputnilai/mapel/1')
                  .then(res => {
                       // handle success
                      this.datas = res.data;
        ...
        @foreach ($datasiswa as $ds)
          @foreach ($datajenisnilai as $dj)
          getPeriksamapel{{ $ds->id }}_{{ $dj->id }}: function () {
              axios.get('http://127.0.0.1:8000/api/admin/inputnilai/periksamapel/{{ $ds->nis }}/{{ $dataajar->kelas_nama }}/{{ $dataajar->pelajaran_nama }}/{{ $dj->nama }}/{{ getsettings::semesteraktif() }}')
                  .then(res => {
                       // handle success
                      this.datas{{ $ds->id }}_{{ $dj->id }} = res.data.data;
                      console.log(this.datas{{ $ds->id }}_{{ $dj->id }});
                  })
                  .catch(err => {
                      // handle error
                      console.log(err);
