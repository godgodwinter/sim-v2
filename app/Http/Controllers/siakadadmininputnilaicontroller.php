...
class siakadadmininputnilaicontroller extends Controller
{
    ...
    public function inputnilaiajax(kelas $kelas){
        // dd($kelas);
        if($this->checkauth('admin')==='404'){
            return redirect(URL::to('/').'/404')->with('status','Halaman tidak ditemukan!')->with('tipe','danger')->with('icon','fas fa-trash');
        }
        ...
        return view('siakad.admin.inputnilai.kepribadian_index_inputajax',compact('pages','jmldata','kelas','datasiswa','datakepribadian','dataekstrakulikuler'));
    }
    ...
    public function ekstra_storeajax(ekstrakulikuler $ekstrakulikuler,kelas $kelas,siswa $siswa,Request $request){
        // dd($dataajar);
        $request->validate([
            'nilai'=>'required|numeric|min:1|max:100',
    ...
    public function kepribadian_storeajax(kepribadian $kepribadian,kelas $kelas,siswa $siswa,Request $request){
        // dd($dataajar);
        $request->validate([
            'nilai'=>'required|numeric|min:1|max:100',
